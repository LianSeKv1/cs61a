...
def depth(t):
    if t.is_leaf():
        return 0
    return 1 + max([depth(item) for item in t.branches])
...
def reverse_other(t):
    ...
    branches_l = len(t.branches)

    if depth(t) % 2 == 1:
        for i in range(branches_l // 2):
            temp = t.branches[i].label
            t.branches[i].label = t.branches[branches_l - 1 - i].label
            t.branches[branches_l - 1 - i].label = temp
