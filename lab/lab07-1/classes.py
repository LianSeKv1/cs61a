...
class ProfessorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        for item in player.deck.cards:
            item.attack = item.attack + opponent_card.attack
            item.defense = item.defense + opponent_card.defense

        
        opponent.deck.cards = [i for i in opponent.deck.cards if i.attack != opponent_card.attack and i.defense != opponent_card.defense]
        i = 0
        j = len(opponent.deck.cards)   
        while i < j:
           if opponent.deck.cards[i].attack == opponent_card.attack or opponent.deck.cards[i].defense == opponent_card.defense:
