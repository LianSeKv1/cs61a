...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive:
            # 当前位置有蜜蜂，则随机返回蜜蜂
            if len(current_place.bees) and self.lower_bound <= attack_p <= self.upper_bound:
                   return random_bee(current_place.bees)
            # 当前位置没有蜜蜂，继续向后移动, 攻击范围更新
            attack_p = attack_p + 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # 如果fireAnt死亡，则增大伤害值
        if self.health <= 0:
            amount = self.damage + amount

        # 当前fireAnt 对所在的place中的所有bees造成伤害
        for item in bees_copy:
            item.reduce_health(amount)
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        "*** YOUR CODE HERE ***"
        # 附魔减速debuff
        target.slow_count = self.slow_count
        ...
        def slow_action(self,gamestate,action = target.action):
            if self.slow_count:
                self.slow_count -= 1
                if gamestate.time % 2 == 0: 
                    action(self,gamestate)

        if target.slow_count == 0:
            target.slow_count = self.slow_count
            target.action = slow_action
