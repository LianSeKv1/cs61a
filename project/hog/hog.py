...
def more_boar(player_score, opponent_score):
    ...
    # 递减到两位
    while (player_score // 100) != 0:
          player_score //= 10
    while (opponent_score // 100) != 0:
          opponent_score //= 10
    # 进行判断
    player_score_1 = player_score//10%10
    opponent_score_1 = opponent_score//10%10
    if(player_score_1>=opponent_score_1):
        return False
    player_score_1 = player_score%10
    opponent_score_1 = opponent_score%10
    if(player_score_1>=opponent_score_1):
        return False
    return True
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_averaged_score = 0   # 最大平均得分
    max_averaged_score_index = 0
    i = 1
    while i <= 10 and max_averaged_score_index <=10:
        score = averaged_dice(i,dice)
        if  score > max_averaged_score :
             max_averaged_score = score
             max_averaged_score_index = i
        i+=1
    # END PROBLEM 9
    return  max_averaged_score_index
...
def piggypoints_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    it  =  piggy_points(opponent_score)
    if it >= cutoff:
        return 0
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # 如果roll 0次触发猪，就抛0次
    it = more_boar(piggy_points(opponent_score)+score,opponent_score)
    if it or piggypoints_strategy(score,opponent_score,cutoff,num_rolls)==0:
        return 0
    return num_rolls

       
    # more_boar()    return 6  # Replace this statement
    # END PROBLEM 11
