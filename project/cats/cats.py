...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    filter_list = [item for item in paragraphs if select(item)]
    return  filter_list[k:k+1] and filter_list[k:k+1][0] or '' 
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def about_inner(paragraphs):
        paragraphs = remove_punctuation(lower(paragraphs)).split()
        for i in topic:
            for j in paragraphs:
                if i == j :
                    return True
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
     # 如果typed_word包含在word_list中直接返回typed_word
    if typed_word in word_list:
        return typed_word

    diff_min = [diff_function(typed_word,item,limit) for item in word_list if diff_function(typed_word,item,limit) <= limit]
    if len(diff_min) == 0:
        return typed_word
    for item in word_list:
        if diff_function(typed_word,item,limit) == min(diff_min):
            return item
...
def time_per_word(words, times_per_player):
    ...
    for i in range( len(times_per_player) ):
        ...
        for j in range(1,len(times_per_player[i])):
                l[i].append(times_per_player[i][j] - times_per_player[i][j - 1])    
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    l  = [[] for i in player_indices]
    for i in word_indices:
        times = [time(match,j,i) for j in player_indices]
        l[times.index(min(times))].append(get_word(match,i))
    return l
    # END PROBLEM 10
