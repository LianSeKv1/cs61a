...
class VirFib():
    ...
    def next(self):
        "*** YOUR CODE HERE ***"
        next_vir = VirFib(self.value and self.value + self.prev_value or 1)
        next_vir.prev_value = self.value
        return next_vir

    def __repr__(self):
        return "VirFib object, value " + str(self.value)
def bst_min(t):
    l = [bst_max(b) for b in t.branches]
    l.append(t.label)
    return min(l)
...
def is_bst(t):
    ...
    if len(t.branches) > 2:
        ...
        return bst_max(t.branches[0]) < bst_min(t.branches[1]) and is_bst(t.branches[0]) and is_bst(t.branches[1])
