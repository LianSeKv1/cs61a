...
def replace_thor_at_leaf(t, thors_replacement):
    ...
    #     return tree(tree(label(t)))
    return tree(is_leaf(t) and label(t)=='thor' and thors_replacement or label(t),[replace_thor_at_leaf(b,thors_replacement) for b in branches(t)])
...
def has_path(t, word):
    ...
    if is_leaf(t) or len(word)==1:
        return len(word)==1 and word == label(t)
    return word[0]==label(t) and len([1 for b in branches(t) if has_path(b,word[1:])])>0
def preorder(t):
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) < 0 and upper_bound(y) < 0 or lower_bound(y) > 0
